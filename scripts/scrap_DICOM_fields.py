...
def scrap_profiles(url):
    ...
    headers = [
        th.text
        for th in soup.find(attrs={"id": "table_E.1-1"})
        .parent.find("table")
        .find("thead")
        .find_all("strong")
    ]
    ...
    for tr in (
        soup.find(attrs={"id": "table_E.1-1"})
        .parent.find("table")
        .find("tbody")
        .find_all("tr")
    ):
        tmp = {
            ...
            for key, value in dict(zip(headers, tr.find_all("td"))).items()
            if key in ["Attribute Name", "Tag", "Basic Prof."]
        }
